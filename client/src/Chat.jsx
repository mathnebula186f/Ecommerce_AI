...
export default function Chat() {
  ...
  async function sendMessage(ev) {
    ...
    try {
      const response1 = await fetch(
        "https://e-commerce-chatbot.onrender.com/predict",
        {
          method: "POST",
  ...
  async function resetChatFunction() {
    console.log("reset fxn called");
    try {
      const response = await axios.post("/resetChat", {
        userId1:"664a5eff7a33c4c28b6574d7",
      ...
      else console.log("Not able to reset the chat,",response.status);
    } catch (error) {
      console.log("Error while reseting chat=",error);
    }
      ...
      <div className="bg-white w-1/3">
        ...
        >
          ...
          <div className="flex gap-2 py-2 pl-4 items-center">
            ...
            <span className="text-gray-800 sm:text-base text-sm">Chatbot</span>
          </div>
        </div>
        <div
          className="flex gap-2 my-10 items-center justify-center bg-green-500 border sm:m-8 rounded-xl w-fit p-2 text-sm"
          onClick={logout}
        >
              ...
              <div className="z-1 overflow-y-scroll absolute inset-0">
                {messagesWithoutDupes.map((message) => (
                  ...
                  >
                    ...
                    >
                      {message.text}
                      <p className="text-xs opacity-50"> {new Date(message.createdAt).toLocaleString()}</p>
        ...
        {!!selectedUserId && (
          <form className="flex gap-2" onSubmit={sendMessage}>
            <input
              ...
              className="bg-white flex-grow border rounded-sm p-2 w-2 sm:w-auto text-sm sm:text-base"
            />
            <button
              type="submit"
            ...
            <button
              type="button"
              className="bg-red-500 p-2 text-white rounded-sm flex items-center text-sm sm:text-base"
              onClick={resetChatFunction} // replace `resetChatFunction` with the actual function name
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className="w-4 h-4 mr-1 sm:w-6 sm:h-6"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
